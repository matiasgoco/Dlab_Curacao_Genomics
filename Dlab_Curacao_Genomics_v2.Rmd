...
salloc --mem=16G --cpus-per-task=2 --time=2:00:00 --partition=cpu # Increase resources and time
module load uri/main
...
salloc --mem=16G --cpus-per-task=2 --time=2:00:00 --partition=cpu
...
NOTE for MOLE 2023: The version of PAUP* running on the MOLE virtual machines has a bug that sometimes causes crashes when multithreading is used with SVDQuartets. (This is probably a long-standing bug that we are are just noticing on very fast CPUs.) To avoid this problem, we will specify “nthreads=1” to suppress multithreading when running SVDQuartets. This is only a temporary limitation, and this notice will be removed once the bug is fixed. 


```{bash}
svdq partition=none seed=475839 bootstrap=standard nreps=100 evalQuartets=all treefile=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq.tree showScores=yes nthreads=1
```


Let's try with evalQuartets=random

```{bash}
svdq partition=none seed=475839 bootstrap=standard nreps=100 evalQuartets=random treefile=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq.tree showScores=yes
```
...
to quit paup just type in quit
...
From previously used script to run SVDquartets:


Quartets in PAUP
I was not able to install PAUP in the Cluster so I downloaded it and ran it in my computer.
