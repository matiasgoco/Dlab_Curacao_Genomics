...
Changing colors to a blind friendly palette

Spring: #009e73 (Green)
Autumn: #e69f00 (Orange)
Both seasons: #cc79a7 (Pink)
Unknown season: #0072b2 (Dark blue)

```{r, eval=FALSE}
cols_season <- (values=c( "#e69f00","#cc79a7","#009e73","#0072b2"))
cols_season
```
...
Dlab_PCA <- p + 
  ...
  theme(text = element_text(size = 20, face = "plain"),
        legend.position = c(0.55, 0.7),  # Center the legend
        ...
        legend.box.margin = margin(8, 8, 8, 8),  # add padding inside the background box
        panel.border = element_rect(color = "black", fill = NA, size = 1.5))  # match thickness
...
```{r}
cols_dapc <- (values=c("#e69f00","#009e73"))
cols_dapc
```
...
setwd("/Users/matiasgcvc/Desktop/Dlab_Reanalysis/Reanalysis_v2/Figures_MS/Blind_Friendly_Figures_Dlab_2025")
ggsave("Dlab_combined_plot_v2.pdf", 
       plot = Dlab_combined, 
       width = 30, 
       height = 45, 
       dpi = 600, 
...
ggsave("Dlab_combined_plot_v2.png", 
       plot = Dlab_combined, 
       width = 30, 
       height = 45, 
       dpi = 600, 
...
## Remove sample 420 from diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000.recode.vcf
...
Since we ended up with the exact same dataset, there's no need to redo any pop gen analyses here. Let's move to the phylogeny inference part with SVDquartets

Following https://github.com/ForBioPhylogenomics/tutorials/blob/main/species_tree_inference_with_snp_data/README.md

## SVDquartets

Input file must be in nexus format


```{bash,  eval=FALSE}
cd /project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review
```
...
PAUP* does not accept VCF files as input, we'll need to convert file NC_031969.f5.sub4.vcf into Nexus format. We can use the Ruby script convert_vcf_to_nexus.rb
...
Make a symbolic link of the VCF

```{bash,  eval=FALSE}
ln -s ../diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.vcf .
```
...
```{bash,  eval=FALSE}
ruby convert_vcf_to_nexus.rb diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.vcf diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.nex
...
```{bash, eval=FALSE}
Reading 90930_16... done.
Preparing the nexus string... done.
Writing the nexus file diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.nex... done.
```


Let's check out diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.nex... 

```{bash,  eval=FALSE}
head diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.nex
```
...
BEGIN DATA;
        DIMENSIONS NTAX=35 NCHAR=1481;
        FORMAT DATATYPE=DNA MISSING=N GAP=- ;

MATRIX

440_dip_sorted           
462_dip_sorted           
```


```{bash,  eval=FALSE}
tail -n 3 diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.nex
```
...
```{bash}
D_lab_UK_6_sperm_sorted  
;
END;
```
...
```{bash,  eval=FALSE}
cd /project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review/SVDquartets/
```

```{bash,  eval=FALSE}
#salloc # TO RUN ON INTERCATIVE NODE
salloc --mem=16G --cpus-per-task=2 --time=4:00:00 --partition=cpu # Increase resources and time
module load uri/main
...
To start PAUP just type in paup

```{bash,  eval=FALSE}
paup
```

now you should see: paup>

The to load your file run
https://www.phylosolutions.com/tutorials/ssb2018/svdquartets-tutorial.html:
Start PAUP and load the data file. Using the command-line interface, type:
The first command argument above is the name of the data file, and the "-L" option opens a log file that will preserve all of the output of your run (you can use any name you like for the log file).


```{bash,  eval=FALSE}
# log file=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.log
log file=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode_ALL_Quartets.log
```
...
"/project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review/SVDquartets/diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_L
D1000_35ind.recode.log".
```
...
Logging output to file
"/project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review/SVDquartets/diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.r
ecode_ALL_Quartets.log".
```



```{bash,  eval=FALSE}
execute diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.nex 
```
...
Processing of file
"/project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review/SVDquartets/diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_L
D1000_35ind.recode.nex" begins...

Data read in DNA format

Data matrix has 35 taxa, 1481 characters
...
Processing of input file "diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.nex" completed.
```


https://github.com/ForBioPhylogenomics/tutorials/blob/main/species_tree_inference_with_snp_data/README.md : 
...
To find out the list of individuals in paup
...
Taxon-status summary:
   Original data matrix contains 35 taxa
   No taxa have been deleted
   No taxa have been assigned to the outgroup: outgroup defaults to first taxon (440 dip sorted)

Current status of all taxa:
...
21 424 dip sorted           -
22 438 dip sorted           -
23 D lab 04 sperm sorted    -
24 D lab 09 tissue sorted   -
25 D lab 10 tissue sorted   -
26 D lab 11 tissue sorted   -
27 D lab 33 tissue sorted   -
28 D lab 34 sperm sorted    -
29 D lab 36 tissue sorted   -
30 D lab 37 tissue sorted   -
31 D lab A  tissue sorted   -
32 D lab C sperm sorted     -
33 D lab E  sperm sorted    -
34 D lab UK 1 sperm sorted  -
35 D lab UK 6 sperm sorted  -
```

To start off with, we do not have a real outgroup as everything comes from the same species and we did not sequence any other taxa here

https://www.asc.ohio-state.edu/kubatko.2//phylogenomics2019/SpeciesTreeTutorial.html
...
Let's run SVDquartets with no specified outgroup (1st taxon as outgroup by default: 440 dip sorted). Also, no partition chosen, that is no lineage specified, 
...
https://molevolworkshop.github.io/faculty/kubatko/pdf/species-trees-tutorial-2023.html:
...
```{bash}
# svdq partition=none seed=475839 bootstrap=standard nreps=100 evalQuartets=all treefile=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq.tree showScores=yes
```


Let's try to run on the intearctive node with more resources allocated

```{bash}
# salloc --mem=24G --cpus-per-task=4 --time=2:00:00 --partition=cpu
salloc --mem=16G --cpus-per-task=2 --time=4:00:00 --partition=cpu
```

It's still taking a while so trying to run it much faster using evalQuartets=random, showScores=no, nthreads=2

```{bash}
#svdq partition=none seed=475839 bootstrap=standard nreps=100 evalQuartets=random treefile=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq.tree showScores=no nthreads=2
```


 it ran in just ten minutes

Let's see if we can run it with evalQuartets=random but still using 2 threads and not showing scores

```{bash}
# svdq partition=none seed=475839 bootstrap=standard nreps=100 evalQuartets=all treefile=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq.tree showScores=no nthreads=2
```

```{bash}
svdq partition=none seed=475839 bootstrap=standard nreps=1000 evalQuartets=all treefile=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.ALL_Quartets_1000_Boots.tree showScores=no nthreads=2
```

It seems to have worked. It worked well

Now, let's make a consensus tree to have bootstrap values displayed on Itol or figtree
https://isu-molphyl.github.io/EEOB563/computer_labs/lab2/PAUP.html
```{bash,  eval=FALSE}
module load uri/main
module load PAUP/4.0a168-centos64
```
...
```{bash,  eval=FALSE}
#log file=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode_Consensus_tree.log
log file=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode_ALL_Quartets_1000_Boots_Consensus_tree.log
```
...
Logging output to file
"/project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review/SVDquartets/diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_L
D1000_35ind.recode_Consensus_tree.log".
```
...
Logging output to file
"/project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review/SVDquartets/diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.r
ecode_ALL_Quartets_1000_Boots_Consensus_tree.log".
```
...
```{bash,  eval=FALSE}
#execute diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq.tree
execute diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.ALL_Quartets_1000_Boots.tree
```
...
Processing of file
"/project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review/SVDquartets/diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_L
D1000_35ind.recode.svdq.tree" begins...

100 trees read from TREES block; time used = 0.01 sec (CPU time = 0.01 sec)

Processing of input file "diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq.tree" completed.
```
...
indices         no|yes                                             no
showTree        no|yes                                             yes
treeFile        <output-treefile-name>                             none
saveAsRooted    no|yes                                             no
saveSupport     no|brlens|nodeLabels|Both                          nodeLabels
format          Nexus|altNexus|Freqpars|Newick|Hennig|Phylip       none
replace         no|yes                                            *no
append          no|yes                                            *no
tcompress       no|yes                                             no
rootMethod      outgroup|lundberg|midpoint                         outgroup
outRoot         polytomy|paraphyletic|monophyletic                 polytomy
forcePolytomy   no|yes                                             no
                                                                  *Option is nonpersistent
```
...
```{bash,  eval=FALSE}
# contree all/strict=no majrule=yes saveAsRooted=no useTreeWts=yes saveSupport=nodeLabels format=Newick treefile=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq_Consensus_Newick.tree
```

```{bash,  eval=FALSE}
contree all/strict=no majrule=yes saveAsRooted=no useTreeWts=yes saveSupport=nodeLabels format=Newick treefile=diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.ALL_Quartets_1000_Boots_Newick.tree
```
...
/project/pi_prada_uri_edu/mgomez/curaçao/snp_filtering/analysis_v2/analysis_v2_review/SVDquartets/diploria2_SNPs_2alleles_30phred_30x_50missgen_50missind_NoClones_maf05_hwe01_LD1000_35ind.recode.svdq_Consensus_Newick.tree
```

Now work on itol for tree editing
