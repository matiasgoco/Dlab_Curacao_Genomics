...
All analyses were conducted in R (version 4.1.1, 2021-08-10). This markdown file outlines each step to assess genetic structure from genotyping analysis using custom SNPsaurus scripts to generate a VCF for the brain coral (*Diploria labyrinthiformis*) and two spawning populations in Curaçao (Southern Caribbean).
...
## Manually assign colors to each season Spring (#01B050), Both (#CC0099), Autumn (#FF9833), Unknown (#000000)

```{r}
cols_dapc <- (values=c("#FF9833","#01B050"))
cols_dapc
```
...
graph_1+geom_line()+scale_x_continuous(breaks=c(1:8))+
